...
export function MessageInterface({ initialMessages, member }: MessageInterfaceProps) {
  ...
  useEffect(() => {
    const fetchRecentMessages = async () => {
      try {
        const freshMessages = await getConversation(member.member_id, adminId);
  ...
  }, [member.member_id, adminId, messages.length]);

  useEffect(() => {
    const groupMessagesByDate = (msgs: Message[]) => {
      const groups: { [key: string]: Message[] } = {};
      msgs.forEach(message => {
        const safeDate = message.created_at || new Date().toISOString();
        ...
        else key = formatDate(safeDate, { month: 'long', day: 'numeric', year: 'numeric' });
                  ...
                  <div key={date} className="relative py-4">
                    ...
                    <div className="mt-4 space-y-6">
                      {msgs.map((message, index) => {
                        ...
                        return (
                          <div key={`${message.id || 'msg'}-${index}`} className={cn('flex items-end gap-2 group', isSender ? 'justify-end' : 'justify-start')}>
                            {!isSender && (
