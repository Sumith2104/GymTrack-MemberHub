...
export default async function WorkoutTrackingPage({
  ...
  searchParams: Promise<{ memberId?: string; email?: string }>;
}) {
  const resolvedParams = await searchParams;
  const memberDisplayId = resolvedParams?.memberId;
  ...
  const [workouts, bodyWeightLogs, checkins] = await Promise.all([
    getMemberWorkouts(member.id),
    getMemberBodyWeightLogs(member.id),
    getMemberCheckins(member.member_id)
  ]);

  const personalRecords = await calculatePersonalRecords(workouts);
  const streak = await calculateWorkoutStreak(checkins);
