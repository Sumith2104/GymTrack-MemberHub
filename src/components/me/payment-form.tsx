...
export function PaymentForm({ member }: PaymentFormProps) {
  ...
  const baseUpiLink = (member.payment_id && planPrice > 0)
    ? `upi://pay?pa=${member.payment_id}&pn=${encodeURIComponent(member.gym_name || 'Gym Payment')}&am=${Number(planPrice).toFixed(2)}&cu=INR`
    : null;
              ...
              <DialogContent className="sm:max-w-md">
                ...
                <div className="flex flex-col items-center justify-center pt-4 space-y-4">
                  ...
                  <p className="text-sm text-muted-foreground text-center">
                    Total Amount: <span className="font-bold">₹{Number(planPrice).toFixed(2)}</span>
                  </p>
                </div>
